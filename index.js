...
document.addEventListener('click', function(e){
    ...
    else if (e.target.id === 'reply-btn') {handleReplyBtnClick()}
})

function handleLikeClick(tweetId){
    const targetTweetObj = tweetsData.filter(function(tweet) {
        return tweet.uuid === tweetId})[0]
            if (targetTweetObj.isLiked) {targetTweetObj.likes--}
...
function handleRetweetClick(tweetId) {
    const targetTweetObj = tweetsData.filter(function(tweet) {
        return tweet.uuid === tweetId})[0]
            if (targetTweetObj.isRetweeted) {targetTweetObj.retweets--} 
...
function handleTweetBtnClick(){
    ...
    if (tweetInput.value) {
    tweetsData.unshift({handle: `@JerkPhil ✅`,
    profilePic: `images/philip.jpg`,
    likes: 0,
    retweets: 0,
...
function getFeedHtml() {
    ...
    tweetsData.forEach(function(tweet) {
       ...
       feedHtml += `<div class="tweet">
        ...
        ${repliesHtml}
        <textarea id="reply-input" placeholder="Tweet your reply"></textarea>
